...
def build_training_cmd(cfg) -> str:
    ...
    master_port = 29500 + int(cfg.experiment_dir.split('/')[-1][:4])
    executor = f'torchrun --monitor-interval 5 --max-restarts {cfg.max_restarts}{" --standalone" if cfg.standalone else ""} --master-port={master_port} --nproc-per-node={cfg.num_gpus}' if cfg.num_gpus > 1 else cfg.env.python_bin
