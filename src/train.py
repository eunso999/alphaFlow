...
@record
@hydra.main(config_path="..", config_name="experiment_config.yaml", version_base=None)
def train(cfg: DictConfig):
    ...
    nets, optimizers, loss_fn, val_loss_fn, vis, stats = network_utils.init_modules(cfg=cfg, device=device, start_time=_start_time, data=data, resume_cfg=resume_cfg)
    ...
    while not stats.done:
        ...
        if (cfg.training.freqs.save_samples is not None) and (stats.done or stats.cur_step % cfg.training.freqs.save_samples == 0) or ctrl_state.should_save_samples:
            log_generated_samples(cfg, nets.ema or nets.net, vis, data.dataset_val, stats, device)
        if (cfg.training.freqs.loss_per_sigma is not None) and (stats.done or stats.cur_step % cfg.training.freqs.loss_per_sigma == 0):
            log_per_sigma_losses(loss_fn, data.dataset_val, stats, cfg, nets.net, device, vis.sigma)
        if (cfg.training.freqs.loss_val is not None) and (stats.done or stats.cur_step % cfg.training.freqs.loss_val == 0):
            if cfg.training.val_with_next_iter:
                num_val_batches = max(cfg.training.num_samples_for_val_loss // (cfg.dataset.test_batch_gpu * dist.get_world_size()), 1)
                val_batches = [next(data.iterator) for _ in range(num_val_batches)]
                log_val_losses(val_loss_fn, val_batches, stats, cfg, nets.net, device)
                if cfg.val_loss.get("is_grad_csim", False):
                    log_val_grad_cosine_sim(val_loss_fn, val_batches, stats, cfg, nets.net, device)
                object.__setattr__(data, 'iterator', itertools.chain(iter(val_batches), data.iterator))
            else:
                log_val_losses(val_loss_fn, data.dataset_val, stats, cfg, nets.net, device)
                if cfg.val_loss.get("is_grad_csim", False):
                    log_val_grad_cosine_sim(val_loss_fn, data.dataset_val, stats, cfg, nets.net, device)
        ...
        with torch.profiler.record_function('stats_log_and_update'):
            ...
            if cfg.wandb.enabled:
                ...
                if cfg.loss.name == 'alphaflow':
                    net_progress_stats = {**net_progress_stats, **{'Progress/alpha': loss_fn.get_ratio(cfg.loss.alpha, stats.cur_step)}}
                    net_progress_stats = {**net_progress_stats, **{'Progress/ratio_fm': loss_fn.get_ratio(cfg.loss.ratio_fm, stats.cur_step)}}
...
def compute_metrics(cfg, metrics_to_compute, metrics_all, data: Data, net, snapshot_path, device, stats) -> float:
    ...
    for sampling_cfg_overwrite_name, sampling_cfg_overwrite_values in sampling_cfg_overwrites.items():
        ...
        for metric in metrics_to_compute:
            result_dict = metric_main.compute_metric(
                metric=metric,
                net=net,
                batch_gen=cfg.dataset.gen_batch_gpu,
                detector_batch_gpu=cfg.dataset.metrics_detector_batch_gpu,
                verbose=True,
            ...
            if cfg.training.main_metric == '__pick_first__' and metric == metrics_all[0]:
...
@torch.no_grad()
def log_val_losses(loss_fn, dataset_val_or_batches, stats, cfg, net, device):
    if not cfg.wandb.enabled or cfg.training.num_samples_for_val_loss == 0:
        return
    dist.loginfo0('Logging validation losses... ', end='')
    loss_dict = compute_val_losses(loss_fn, dataset_val_or_batches, stats, cfg, net, device, cfg.training.num_samples_for_val_loss)
...
def _accumulate_grad_norms(losses_val: dict, params: list, device: torch.device, grad_norm_values: dict, suffix: str) -> None:
    """Forward pass 결과로부터 각 loss key의 gradient L2 norm을 계산해 grad_norm_values에 누적.

    retain_graph=True를 활용해 단일 forward graph를 재사용하므로 forward는 한 번만 수행.
    마지막 loss key의 backward에서 graph를 해제한다.
    """
    loss_keys = list(losses_val.keys())
    for i, loss_key in enumerate(loss_keys):
        if "total" in loss_key:
            retain = (i < len(loss_keys) - 1)
            grads = torch.autograd.grad(
                losses_val[loss_key].mean(), params,
                allow_unused=True, retain_graph=retain, create_graph=False,
            )
            flat_grad = torch.cat([
                g.detach().reshape(-1) if g is not None else torch.zeros(p.numel(), device=device)
                for g, p in zip(grads, params)
            ])
            grad_norm_values[f"{loss_key}{suffix}"].append(flat_grad.norm().item())
            del flat_grad, grads


@torch.no_grad()
def compute_val_losses(loss_fn, dataset_val_or_batches, stats, cfg, net, device, num_samples_to_use: int):
    misc.free_memory()
    if isinstance(dataset_val_or_batches, list):
        batches = dataset_val_or_batches
        dataset_val_iterator = iter(batches)
        num_batches_to_process = len(batches)
        cleanup_vars = []
    else:
        dataset_val = dataset_val_or_batches
        ...
        cleanup_vars = [dataset_val, dataset_val_cur, dataloader_val]
    ...
    log_grad_norm_var = cfg.val_loss.get('is_grad_norm', False)
    grad_norm_values = defaultdict(list) if log_grad_norm_var else None  # {key: [grad_norm_per_batch]}
    params = [p for p in net.parameters() if p.requires_grad] if log_grad_norm_var else None
    for _ in range(num_batches_to_process):
        ...
        for delta_size in [0.05, 0.16, 0.29, 0.45, 0.68]:
            loss_fn.cfg.distrib_t_t_next_mf.time_sampling_mf_t.min = delta_size
            loss_fn.cfg.distrib_t_t_next_mf.time_sampling_mf_t_next.delta_size = delta_size

            is_mf_t_constant = (loss_fn.cfg.distrib_t_t_next_mf.time_sampling_mf_t.timestep_distrib_type == "constant")
            if loss_fn.cfg.get("loss_per_t", False) and is_mf_t_constant:
                t_list = [i / 20 for i in range(21)][1:]
                for t in t_list:
                    loss_fn.cfg.distrib_t_t_next_mf.time_sampling_mf_t.scale = t
                    if log_grad_norm_var:
                        with torch.enable_grad():
                            losses_val = loss_fn(net=net, x=x, cond=cond, cur_step=stats.cur_step, phase=LossPhase.Gen) # [num_val_samples, num_pyr_levels]
                            _accumulate_grad_norms(losses_val, params, device, grad_norm_values, suffix=f"_t:{t}_delta:{delta_size}")
                    else:
                        losses_val = loss_fn(net=net, x=x, cond=cond, cur_step=stats.cur_step, phase=LossPhase.Gen) # [num_val_samples, num_pyr_levels]
                    for loss_key, loss_val in losses_val.items():
                        loss_val_values[f"{loss_key}_t:{t}_delta:{delta_size}"].append(loss_val.mean(dim=0).detach().cpu())
            else:
                if log_grad_norm_var:
                    with torch.enable_grad():
                        losses_val = loss_fn(net=net, x=x, cond=cond, cur_step=stats.cur_step, phase=LossPhase.Gen) # [num_val_samples, num_pyr_levels]
                        _accumulate_grad_norms(losses_val, params, device, grad_norm_values, suffix=f"_delta:{delta_size}")
                else:
                    losses_val = loss_fn(net=net, x=x, cond=cond, cur_step=stats.cur_step, phase=LossPhase.Gen) # [num_val_samples, num_pyr_levels]
                for loss_key, loss_val in losses_val.items():
                    loss_val_values[f"{loss_key}_delta:{delta_size}"].append(loss_val.mean(dim=0).detach().cpu())
    loss_dict = {}
    ...
    # Gradient norm의 배치 간 분산 집계: 각 GPU의 norm 리스트를 all_gather 후 mean/var 계산.
    # 분산이 클수록 배치 간 gradient 크기 변동이 크다 → 학습 불안정 지표.
    # wandb에서 mean, mean+std, mean-std를 같은 차트에 올리면 불안정 구간을 밴드로 시각화할 수 있다.
    if log_grad_norm_var:
        clip_threshold = cfg.model.grad_clip.norm  # None이면 clipping 비활성
        loss_scaling = cfg.model.loss_scaling       # grad_scaler는 unscale로 상쇄되므로 이것만 적용
        for gnorm_key, gnorm_list in grad_norm_values.items():
            gnorm_tensor = torch.tensor(gnorm_list, device=device)  # [num_batches]
            if torch.distributed.is_initialized():
                gathered = [torch.zeros_like(gnorm_tensor) for _ in range(dist.get_world_size())]
                torch.distributed.all_gather(gathered, gnorm_tensor)
                gnorm_tensor = torch.cat(gathered)  # [num_gpus * num_batches]
            gnorm_mean = gnorm_tensor.mean().item()
            gnorm_std = gnorm_tensor.std().item()
            loss_dict[f'Grad/val_{gnorm_key}_mean'] = gnorm_mean
            loss_dict[f'Grad/val_{gnorm_key}_mean_plus_std'] = gnorm_mean + gnorm_std
            loss_dict[f'Grad/val_{gnorm_key}_mean_minus_std'] = gnorm_mean - gnorm_std
            # 학습의 Grad/norm과 비교 가능한 스케일: loss_scaling × ||grad(loss.mean())||
            # (grad_scaler는 unscale_ 후 상쇄되므로 무시, num_accum_rounds는 val에서 1)
            if gnorm_key.startswith('total'):
                scaled_mean = loss_scaling * gnorm_mean
                if clip_threshold is not None:
                    loss_dict[f'Grad/val_{gnorm_key}_clip_ratio'] = scaled_mean / clip_threshold
    torch.distributed.barrier() # Setting a barrier for a longer timeout to make sure that all the samples are processed.
    del dataset_val_iterator, loss_val_values, loss_val_agg
    for _v in cleanup_vars: del _v
    misc.free_memory()
    return loss_dict

#----------------------------------------------------------------------------

def compute_val_grad_cosine_sim(loss_fn, dataset_val_or_batches, stats, cfg, net, device, num_samples_to_use: int) -> dict[str, float]:
    """Compute per-batch cosine similarities between gradients of various losses at different delta values.

    For each pair, reports mean / 5th-percentile / 95th-percentile across batches (and GPUs).

    Returns a dict whose keys are '{pair_name}/{mean|p05|p95}'.
    """
    misc.free_memory()
    if isinstance(dataset_val_or_batches, list):
        batches = dataset_val_or_batches
        ...
        num_batches_to_process = max(num_samples_to_use // (cfg.dataset.test_batch_gpu * dist.get_world_size()), 1)
        cleanup_vars = [dataset_val, dataset_val_cur, dataloader_val]
    assert num_batches_to_process > 0, f"num_batches_to_process={num_batches_to_process} is not positive."

    params = [p for p in net.parameters() if p.requires_grad]
    delta_to_loss_keys = {
        0.0:  ['total'],
        0.05: ['total', 'trajectory_consistency'],
        0.68: ['total', 'trajectory_consistency'],
    }
    pairs = [
        ('0.05_vs_0.68',            (0.05, 'total'), (0.68, 'total')),
        ('0.05_tcc_vs_0.68_tcc',    (0.05, 'trajectory_consistency'), (0.68, 'trajectory_consistency')),
        # ('0.0_total_vs_0.05_tcc',   (0.0,  'total'), (0.05, 'trajectory_consistency')),
        # ('0.0_total_vs_0.68_tcc',   (0.0,  'total'), (0.68, 'trajectory_consistency')),
        # ('0.0_total_vs_0.05_fm',    (0.0,  'total'), (0.05, 'trajectory_FM')),
        # ('0.0_total_vs_0.68_fm',    (0.0,  'total'), (0.68, 'trajectory_FM')),
        ('0.0_total_vs_0.05_total', (0.0,  'total'), (0.05, 'total')),
        ('0.0_total_vs_0.68_total', (0.0,  'total'), (0.68, 'total')),
    ]
    cos_sim_lists: dict[str, list[float]] = defaultdict(list)

    for _ in range(num_batches_to_process):
        batch = next(dataset_val_iterator)
        x, cond = construct_inputs_from_batch(batch, use_x_cond=cfg.model.use_x_cond, device=device)
        batch_grads: dict[tuple, torch.Tensor] = {}
        for delta_size, loss_keys in delta_to_loss_keys.items():
            loss_fn.cfg.distrib_t_t_next_mf.time_sampling_mf_t.min = delta_size
            loss_fn.cfg.distrib_t_t_next_mf.time_sampling_mf_t_next.delta_size = delta_size
            with torch.enable_grad():
                losses_val = loss_fn(net=net, x=x, cond=cond, cur_step=stats.cur_step, phase=LossPhase.Gen)
            available_keys = [k for k in loss_keys if k in losses_val]
            for i, key in enumerate(available_keys):
                is_last = (i == len(available_keys) - 1)
                loss = losses_val[key].mean()
                grads = torch.autograd.grad(loss, params, allow_unused=True, create_graph=False, retain_graph=not is_last)
                batch_grads[(delta_size, key)] = torch.cat([
                    g.detach().reshape(-1) if g is not None else torch.zeros(p.numel(), device=device)
                    for g, p in zip(grads, params)
                ]).cpu()
                del grads, loss
        ...
        for name, k1, k2 in pairs:
            if k1 in batch_grads and k2 in batch_grads:
                cs = torch.nn.functional.cosine_similarity(
                    batch_grads[k1].unsqueeze(0), batch_grads[k2].unsqueeze(0)
                ).item()
                cos_sim_lists[name].append(cs)
        del batch_grads

    if torch.distributed.is_initialized():
        for name in list(cos_sim_lists.keys()):
            local_t = torch.tensor(cos_sim_lists[name], device=device)
            gathered = [torch.zeros_like(local_t) for _ in range(dist.get_world_size())]
            torch.distributed.all_gather(gathered, local_t)
            cos_sim_lists[name] = torch.cat(gathered).cpu().tolist()

    results = {}
    for name, values in cos_sim_lists.items():
        t = torch.tensor(values)
        results[f'{name}/mean'] = t.mean().item()
        results[f'{name}/p05']  = torch.quantile(t, 0.05).item()
        results[f'{name}/p95']  = torch.quantile(t, 0.95).item()

    torch.distributed.barrier()
    del dataset_val_iterator, cos_sim_lists
    for _v in cleanup_vars: del _v
    misc.free_memory()
    return results


def log_val_grad_cosine_sim(loss_fn, dataset_val_or_batches, stats, cfg, net, device):
    if not cfg.wandb.enabled or cfg.training.num_samples_for_val_loss == 0:
        return
    dist.loginfo0('Computing gradient cosine similarity... ', end='')
    results = compute_val_grad_cosine_sim(
        loss_fn, dataset_val_or_batches, stats, cfg, net, device, cfg.training.num_samples_for_val_loss
    )
    logging_utils.wandb_log_with_stats0({f'Loss/val_grad_cosine_sim_{k}': v for k, v in results.items()}, stats)
    dist.loginfo0('done! ' + ', '.join(f'{k}={v:.4f}' for k, v in results.items()))
...
def evaluate_and_snapshot(cfg, stats, nets: Nets, optimizers: Optimizers, data: Data, vis, device, should_ctrl_snapshot: bool=False):
    ...
    curr_main_metric_value = compute_metrics(cfg, curr_metrics + curr_metrics_rare + curr_metrics_final, cfg.training.metrics, data, nets.ema or nets.net, snapshot_path, device, stats) # [1]
